...
def data_breaker(storage, alignment_length, break_range=20000):
	""" Function that will break the storage into smaller parts (of 50 000 positions each) and each one will be
	processed separately and then merged in the end. This is only necessary for very large alignments, in which case
	the partitioning prevents the machine from running out of memory, and it's considerably faster """
	subpart_list = []
	previous_value = 0
...
def extremities_filler(sequence, missing_symbol, gap_symbol, extremities="both"):
	""" Function used to substitute the gap filled extremities of aligned sequences with the missing data symbol """
	nucl, nucl_rev = 0, -1  # These are the counter to iterate over the sequence. The nucl counter starts at the
	# beginning, going forward. The nucl_rev starts at the end of the sequence, going backwards
	sequence_list = list(sequence)
	sequence_final = None

	if len(set(sequence_list)) == 1:  # Checks if there are more than 1 type of character. If a sequence is composed
	# only with, let's say, gaps, then the next line replaces it all for the missing data symbol
...
def column_filter(sequence_dict, missing_threshold, gap_threshold, missing_symbol, gap_symbol, multi="no"):
	""" This function checks the columns of the alignment and either removes or masks columns that are not up to the
	...
	for position in range(sequence_length - 1, -1, -1):
		if multi == "no":  # When there are multiple files being processed, do not show the progress bar for each
		# alignment
...
def file_filter(sequences_dict, missing_symbol, gap_symbol, missing_threshold, gap_threshold, multi="no",
				extremities_2="both"):
	...
	for header, sequence in sequences_dict.items():
		new_storage[header] = extremities_filler(sequence, missing_symbol, gap_symbol, extremities=extremities_2)
	# Filters sequences according to the given thresholds
	filter_sequences, excluded_missing, excluded_gaps, sequence_length, total_missing, total_gap = column_filter(
		new_storage, missing_threshold, gap_threshold, missing_symbol, gap_symbol, multi)
...
def master_log(infile, missing_number, gap_number, missing_threshold, gap_threshold):
	...
	log_handle.write("This is the master log for Alignment_filter.py v0.1 run at %s\n\nProcessing file %s\n\nThe number"
					 " of excluded positions due to missing data (threshold was %s):%s\n\nExcluded positions due to "
					 "excessive gap data (threshold was %s): %s" %
					 (time.asctime(time.localtime()), infile, missing_threshold, missing_number, gap_threshold,
					  gap_number))
		...
		if alignment_length > 20000:
			...
			for subpart in subpart_list:
				...
				if subpart == subpart_list[0]:
					...
					file_filter(sub_storage, arg.missing, arg.gap, missing, gap, multi="yes", extremities_2="begin")
				elif subpart == subpart_list[-1]:
					filter_sequences, excluded_missing, excluded_gaps, sequence_length, total_missing, total_gap = \
						file_filter(sub_storage, arg.missing, arg.gap, missing, gap, multi="yes", extremities_2="end")
				else:
					filter_sequences, excluded_missing, excluded_gaps, sequence_length, total_missing, total_gap = \
			...
			if len(input_file_list) == 1:
				filter_sequences, excluded_missing, excluded_gaps, sequence_length, total_missing, total_gap = \
					file_filter(storage, arg.missing, arg.gap, missing, gap)
				...
				loading(input_file_list.index(infile) + 1, len(input_file_list), "Processing alignments (%s)" %
						infile, 50)
				filter_sequences, excluded_missing, excluded_gaps, sequence_length, total_missing, total_gap = \
					file_filter(storage, arg.missing, arg.gap, missing, gap, multi="yes")
				missing_data_number += len(excluded_missing)
				gaps_number += len(excluded_gaps)
				logger(infile, excluded_missing, excluded_gaps, missing, gap)
				dump_fasta(filter_sequences, taxa_order, infile)
